            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-semibold">Editar Orientado</h3>
                        <a href="{{ route('orientados.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                            Voltar
                        </a>
                    </div>

                    <form action="{{ route('orientados.update', $orientado) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-4">
                            <label for="nome_orientado" class="block text-sm font-medium text-gray-700">Nome do Orientado</label>
                            <input type="text" name="nome_orientado" id="nome_orientado" value="{{ old('nome_orientado', $orientado->nome_orientado) }}" 
